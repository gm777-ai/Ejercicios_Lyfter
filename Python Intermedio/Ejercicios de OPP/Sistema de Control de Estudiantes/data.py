    ...
    try:
        with open(CSV_FILE_NAME, mode="r", newline="", encoding="utf-8") as file:
            ...
            for row in reader:
                ...
                if not is_valid_imported_grade(row["spanish"]):
                    skipped_count += 1
                    continue

                if not is_valid_imported_grade(row["english"]):
                    skipped_count += 1
                    continue

                if not is_valid_imported_grade(row["social_studies"]):
                    skipped_count += 1
                    continue

                if not is_valid_imported_grade(row["science"]):
                    skipped_count += 1
                    continue
